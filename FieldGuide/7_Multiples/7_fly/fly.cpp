...
const int SPEED = 3;
const int INITIAL_FLY_COUNT = 10;
...
void draw_game(const game_data &game)
{
    ...
    for (int i = 0; i < length(game.flies); i++)
    {
        // Draw fly
        draw_entity(game.flies[i].ent);
    }
...
void update_game(game_data &game, long current_time)
{
    ...
    // using i looping from 0 to
    for (int i = 0; i < length(game.flies); i++)
    {
        // if collided, score +1
        if (entities_collide(game.spider.ent, game.flies[i].ent))
        {
            game.flies[i] = new_fly(current_time, game.flies[i].ent.bmp);
            game.score++;
        }

        update_fly(game.flies[i], current_time);
    }
}
...
int main()
{
    ...
    {
        add(game.flies, new_fly(current_ticks(), fly_bmp));
    }
